...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    //------------ BEGIN DEEP INTEROP MOD -----------//
    //REASON: we should use the new bundle I guess??
    
    //NSBundle *_extra_unity_bundle = [NSBundle bundleWithURL:[[NSBundle mainBundle] URLForResource:@"iOSUnityBundle" withExtension:@"bundle"]];
    
    //NSLog(@"extra unity bundle URL: %@", [[NSBundle mainBundle] URLForResource:@"iOSUnityBundle" withExtension:@"bundle"]);
    
    //NSLog(@"extra unity bundle path: %s", [[_extra_unity_bundle bundlePath] UTF8String]);
    
    UnityInitApplicationNoGraphics([[[NSBundle mainBundle] bundlePath] UTF8String]);
    //UnityInitApplicationNoGraphics([[_extra_unity_bundle bundlePath] UTF8String]);
    ...
    //------------ BEGIN DEEP INTEROP MOD -----------//
    //REASON: we rely on AppDelegate in native app to create window manually or via storyboard.
    //This means, we need to call didFinishLaunchingWithOptions AFTER window has been created
...
- (void)applicationDidBecomeActive:(UIApplication*)application
{
	...
	else if(!_startUnityScheduled)
	{
        ...
        //------------ BEGIN DEEP INTEROP MOD -----------//
        //REASON: we don't show a splashscreen, we initialize Unity right away and then pause it
...
//------------ BEGIN DEEP INTEROP MOD -----------//
//REASON: we implement convenience methods to pause/resume Unity on the fly
...
- (void) take_pause_path
{
    if(_unityAppReady)
    {
        ...
        //TODO: do we actually need the snapshot functionality?
        _snapshotView = [self createSnapshotView];
        if(_snapshotView)
            [_rootView addSubview:_snapshotView];
    }
